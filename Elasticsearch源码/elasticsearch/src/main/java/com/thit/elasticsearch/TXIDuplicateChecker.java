	...
	public static TXIModel dupliacteCheck(String index, String inputParams, String detailnum, String errorMsg,
		...
		if ("1".equals(flag)) {
			...
			SearchResponse response = searchBuilder.setTypes(tab)
					...
					.setQuery(idsbuliders).setSize(100).get(new TimeValue(60*1000));
			// 遍历命中记录
		...
		if("T_MAT".equalsIgnoreCase(tableName)){
			...
			if(!"".equals(matvoch)){
				...
				if(hitnum>10){
					...
					filter.filter(QueryBuilders.matchQuery("MATDXLG", QueryParser.escape(mattdxlg)).operator(Operator.AND));
