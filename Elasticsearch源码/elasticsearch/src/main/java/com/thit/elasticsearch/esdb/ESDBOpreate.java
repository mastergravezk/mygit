...
public class ESDBOpreate {
	...
	public static TXIModel addDatasToES(String index,String detailnum,String tableName,String pk_col,String errorMsg,TXIModel model) throws Exception{
		...
		while (keys.hasMoreElements()) {
			...
			if (co_id != null) {//有主键的表
				prepareBulk.add(
						client.prepareIndex(index.toLowerCase(), tableName.toLowerCase(), co_id).setSource(addMap));
			}else{//无主键的表
				prepareBulk.add(client.prepareIndex(index.toLowerCase(), tableName.toLowerCase()).setSource(addMap));
			}
			num++;
			System.out.println("发布数据"+co_id);
		}
		System.out.println(tableName+"发布数据条数"+String.valueOf(num));
		if(!detail.isEmpty()){
			BulkResponse bulkResponse = prepareBulk.get();
			if(bulkResponse.hasFailures()){
				System.out.println("同步es数据失败："+bulkResponse.buildFailureMessage());
		...
		while(keys.hasMoreElements()){
			...
			for(String col : keySet){
				...
				if("".equals(val.toString())){
					publishMap.put(col, null);
				}else if("CO_CREATETIME".equals(col)){
					if(val!=null){
						publishMap.put(col, ESUtil.FormatTime(val.toString()));
					}
				}else if("CO_ID".equals(col)){
					co_id = val==null?"":(String)val;
					publishMap.put(col, co_id);
				}else if("CO_MANUALCODE".equals(col)){
					co_manualcode = val==null?"":(String)val;
					publishMap.put(col, co_manualcode);
				}else if("CO_VALID".equals(col)){
					co_valid = val==null?"":(String)val;
					//publishMap.put(col, co_valid);
			...
			if("2".equals(co_freeze)){
				publishMap.put("CO_FREEZE", "0");
			}
			//将所有需要发布的数据放入到BulkRequestBuilder中先不插入ES
			if(!"1".equals(co_valid)){
				publishMap.put("CO_VALID", "1");
			}
			if("".equals(_lang)){
				System.out.println("语言标记："+_lang);
				publishMap.put("CO_ATTRLANGUAGE", lang);
			}else{
				publishMap.put("CO_ATTRLANGUAGE", _lang);
			}
			//将每一条即将发布的数据放到bulk
			if(co_id!=null){
				prepareBulk.add(client.prepareIndex(index.toLowerCase(),tableName.toLowerCase(),co_id)
						.setSource(publishMap));
			}
			num++;
			System.out.println("发布数据："+co_id);
		}
...
//			bulkResponse.buildFailureMessage();
			...
			System.out.println(tableName+"发布"+lang+"数据条数"+String.valueOf(num));
			TXISystem.log.info(tableName+"添加数据：", "ES中添加数据成功！");
		}
		return model;
	}
